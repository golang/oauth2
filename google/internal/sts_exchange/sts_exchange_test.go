...
var exchangeTokenRequest = Request{
	ActingParty: struct {
		ActorToken     string
...
func TestRefreshToken(t *testing.T) {
	...
	resp, err := RefreshToken(context.Background(), ts.URL, refreshToken, auth, headers)
	if err != nil {
		t.Fatalf("exchangeToken failed with error: %v", err)
	}
	...
	resp, err = RefreshToken(context.Background(), ts.URL, refreshToken, auth, nil)
	if err != nil {
		t.Fatalf("exchangeToken failed with error: %v", err)
	}
...
func TestRefreshToken_Err(t *testing.T) {
	...
	_, err := RefreshToken(context.Background(), ts.URL, refreshToken, auth, headers)
	if err == nil {
		t.Errorf("Expected handled error; instead got nil.")
	}
}
