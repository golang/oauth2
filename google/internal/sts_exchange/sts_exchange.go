...
func ExchangeToken(ctx context.Context, endpoint string, request *Request, authentication ClientAuthentication, headers http.Header, options map[string]interface{}) (*Response, error) {
	data := url.Values{}
...
func RefreshToken(ctx context.Context, endpoint string, refreshToken string, authentication ClientAuthentication, headers http.Header) (*Response, error) {
	data := url.Values{}
...
// Request contains fields necessary to make an oauth2 token exchange.
type Request struct {
	ActingParty struct {
		ActorToken     string
